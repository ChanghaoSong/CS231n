...
def sim(z_i, z_j):
    ...
    ##############################################################################
    norm_dot_product=(z_i@z_j)/(torch.linalg.norm(z_i)*torch.linalg.norm(z_j)+1e-8)
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        ##############################################################################
        up=torch.exp(sim(z_k,z_k_N)/tau) #分子
        down=0
        for i in range(2*N):
            if i!=k:
                down+=torch.exp(sim(z_k,out[i])/tau)
        total_loss+=-torch.log(up/down)

        down=0
        for i in range(2*N):
            if i!=k+N:
                down+=torch.exp(sim(z_k_N,out[i])/tau)
        total_loss+=-torch.log(up/down)
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
...
def sim_positive_pairs(out_left, out_right):
    ...
    # HINT: torch.linalg.norm might be helpful.                                  #
    ##############################################################################
    pos_pairs=out_left*out_right #(N,D)
    pos_pairs=torch.sum(pos_pairs,dim=1,keepdim=True) #(N,1)
    pos_pairs/=torch.linalg.norm(out_left,dim=1,keepdim=True)*torch.linalg.norm(out_right,dim=1,keepdim=True)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return pos_pairs
...
def compute_sim_matrix(out):
    ...
    norm=torch.linalg.norm(out,dim=1,keepdim=True)*torch.linalg.norm(out,dim=1,keepdim=True).T
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    pos_sim=torch.concat([pos_sim,pos_sim],dim=0)
